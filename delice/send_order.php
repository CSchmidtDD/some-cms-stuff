...
// EOF - Tomcraft - 2009-10-03 - Paypal Express Modul
  ...
  if ($send_by_admin) {
	  ...
	  $html_mail = $smarty->fetch(CURRENT_TEMPLATE.'/mail/'.$order->info['language'].'/entry_mail.html');
	$txt_mail = $smarty->fetch(CURRENT_TEMPLATE.'/mail/'.$order->info['language'].'/entry_mail.txt');
  }
  //$html_mail = $smarty->fetch(CURRENT_TEMPLATE.'/mail/'.$order->info['language'].'/order_mail.html');
  //$txt_mail = $smarty->fetch(CURRENT_TEMPLATE.'/mail/'.$order->info['language'].'/order_mail.txt');
  ...
  // send mail to customer over admin panel
  ...
  // send mail to customer about normal order  if (SEND_EMAILS == 'true' && $send_by_admin) {
    // if (SEND_EMAILS == 'true' && !$send_by_admin) {
		// xtc_php_mail(EMAIL_BILLING_ADDRESS,
			 // EMAIL_BILLING_NAME,
			 // $order->customer['email_address'],
