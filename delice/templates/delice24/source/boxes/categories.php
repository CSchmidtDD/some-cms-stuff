...
while ($categories = xtc_db_fetch_array($categories_query, true)) {
	$foo[$categories['categories_id']] = array ('name' => htmlspecialchars($categories['categories_name']), 'parent' => $categories['parent_id'], 'level' => 0, 'path' => $categories['categories_id'], 'next_id' => false);
	...
	while (list ($key, $value) = each($id)) {
		...
		if ($category_check > 0) {
			...
			while ($row = xtc_db_fetch_array($categories_query, true)) {
				$foo[$row['categories_id']] = array ('name' => htmlspecialchars($row['categories_name']), 'parent' => $row['parent_id'], 'level' => $key +1, 'path' => $new_path.'_'.$row['categories_id'], 'next_id' => false);
