...
function xtc_pdf_bill ($oID, $send=false, $deliverSlip=false)
{
    ...
    if ($send == true) {
        ...
        $smarty->assign('logo_path', HTTP_SERVER.DIR_WS_CATALOG.'images/');
        ...
        if (PDF_MAIL_SLIP_FORWARDER == 'true' && !$deliverSlip) {
            $mailTemplate = 'sent_mail';
        } else if (PDF_MAIL_SLIP_FORWARDER == 'true' && $deliverSlip) {
            $mailTemplate = 'ls_mail';
        } else {
			$mailTemplate = 'sent_mail';
		}
		...
		if (!$deliverSlip) {
			// NEW
			$attachments[] = DIR_FS_DOCUMENT_ROOT . 'pdf/Widerrufsbelehrung.pdf';
			...
			xtc_php_mail(EMAIL_BILLING_ADDRESS, EMAIL_BILLING_NAME, PDF_MAIL_SLIP_FORWARDER_EMAIL, PDF_MAIL_SLIP_FORWARDER_NAME, '', EMAIL_BILLING_REPLY_ADDRESS, EMAIL_BILLING_REPLY_ADDRESS_NAME, $attachement_filename, '', $mail_subject, $html_mail, $txt_mail);
		}
        // send copy if needed
        ...
        if (PDF_UPDATE_STATUS == 'true') {
			...
			// Send E-Mail to Customer BEGIN
			
			
			
			
			
			// Send E-Mail to Customer END
		}	
